...
void AccountSettings::Init()
{
	...
	ptr = str.GetBuffer(255);
	GetPrivateProfileString(section, _T("EC"), _T("0"), ptr, 256, iniFile);
	str.ReleaseBuffer();
	ec = _wtoi(str);
...
bool AccountSettings::AccountLoad(int id, Account *account)
{
	...
	ptr = str.GetBuffer(255);
	GetPrivateProfileString(section, _T("registerRefresh"), _T(_GLOBAL_ACCOUNT_REGISTER_REFRESH_DEFAULT), ptr, 256, iniFile);
	str.ReleaseBuffer();
	account->registerRefresh = _wtoi(str);
	if (account->registerRefresh < 10) {
		account->registerRefresh = 10;
	}

	ptr = str.GetBuffer(255);
	GetPrivateProfileString(section, _T("keepAlive"), _T(_GLOBAL_ACCOUNT_KEEP_ALIVE_DEFAULT), ptr, 256, iniFile);
	str.ReleaseBuffer();
